...
| BCP vs DRP | BCP=전사적 업무연속성(상위) / DRP=IT복구(하위개념) |
| BCP 5단계 | 범위설정 → BIA → 복구전략개발 → 복구계획수립 → 테스트·유지보수 |
| CC 4대 구성요소 | TOE(평가대상)/PP(보호프로파일)/ST(보안목표명세서)/EAL(1~7등급) |
| 사회공학 2분류 | 인간기반(도청·쓰레기통뒤지기) / 컴퓨터기반(피싱·파밍·스미싱) |
| ISMS-P 인증기준 | 총 102개(관리체계16+보호대책64+개인정보22), ISMS만은 80개 |
| ISMS-P PDCA | Plan-Do-Check-Act |
...
| 개인정보보호법 체계 | 총칙/보호정책수립/처리및안전관리(안전조치의무)/권리보장및구제 |
| 안전성 확보조치 8가지 | 내부관리계획·접근권한제한·접근통제·저장전송·접속기록·악성프로그램점검·물리조치·기타 |

## 【 인프라 프로토콜 & 서비스 】

| 주제 | 정답 / 핵심 |
|---|---|
| DNS 질의 순서 | 로컬캐시 → hosts 파일 → DNS서버 |
| HTTP Connection | close(1.0) vs Keep-Alive(1.1, timeout/max) |
| FTP 포트 | 제어 21/tcp, 데이터 20/tcp(능동모드) / 안전 대안: SFTP(22), FTPS(990) |
| SNMP 포트 | Manager=162/udp(Trap) / Agent=161/udp(Get·Set) |
| DHCP 절차 | Discover→Offer→Request→Ack(DORA), 67/68 udp |
| DHCP Starvation | 가짜 MAC 대량 요청으로 IP풀 고갈 → DHCP Snooping 대응 |
| 이메일 구성요소 | MUA-MTA-MDA-MRA, SMTP(25/tcp) |
| DB 집성(Aggregation) | 낮은등급 정보 조합 → 높은등급 정보 유출 |
| DB 추론(Inference) | 통계 질의로 개별 민감정보 유추 |
| 클라우드 서비스모델 | IaaS(인프라)/PaaS(플랫폼)/SaaS(SW) |
| SECaaS | 클라우드 기반 보안 서비스, CSA 11개 분야 |

## 【 침해사고 분석 및 대응 】

| 주제 | 정답 / 핵심 |
|---|---|
| Snort 룰 구조 | 헤더(Action+프로토콜+IP+포트) + 바디(msg/content/sid) |
| NTP monlist 점검 | `ntpdc -n -c monlist <서버>` |
| NTP 차단 iptables | `-m length --length 100: -j DROP` |
| Nessus / Nikto | 취약점 점검(시스템전반 / 웹특화) |
| Tripwire / chkrootkit | 무결성 점검 / 루트킷 점검 |
| 리버스 쉘 시나리오 | 아웃바운드 허용 악용 + crontab에 netcat 등록 |
| 루트킷 시나리오 | 관리자페이지 노출+기본계정 → 재설치가 원칙 |
| DBD 구조 | 경유지→중계지→유포지, iframe 난독화 |
| 워터링홀 | 표적이 방문하는 사이트를 미리 감염 |
| ShellShock | CVE-2014-6271, CVSS 10점(하트블리드보다 높음) |
| SSDP DDoS | UDP 1900, 멀티캐스트 239.255.255.250, M-SEARCH 증폭 |
